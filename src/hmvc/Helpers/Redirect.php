...
class Redirect {
    ...
    public static function action($url, $data = '', $status = 302, $headers = array()) {
        ...
        switch (count($segments)) {
            case 1:
                $url = baseUrl(app()->get('hmvcDispatch')->getPathController()) . '/' . $url;
                break;
            case 2:
                $url = baseUrl(app()->get('hmvcDispatch')->getPathModule()) . '/' . $url;
                break;
            case 3:
                $url = baseUrl(app()->get('hmvcDispatch')->getPathPrefix()) . '/' . $url;
                break;
            default:
                $url = baseUrl();
                break;
